...
public abstract class AbstractGameObject implements IGameObject, IToJsonObject{
    ...
    int lifes = 1;    
    int mode = 0;
    ConcurrentLinkedQueue<IGameObject> iGameObjects;
    ...
    public AbstractGameObject(Position position, int value, int life, ConcurrentLinkedQueue<IGameObject> gameObjects) {
        this(position);
        this.value = value;
        this.lifes = life;
    ...
    public AbstractGameObject(AbstractGameObject ago){
        position=new Position(ago.position);
        lifes= ago.lifes;
        value= ago.value;
        mode= ago.mode;
    }
    ...
    public static <T extends IGameObject> IGameObject getClosest(Position p, ArrayList<T> jObjs){
        if (jObjs == null || jObjs.isEmpty()){
            System.out.println("prueba de null closest()");
    ...
    public Position moveToNextPosition(){return position;}
    public void setIGameObjects(ConcurrentLinkedQueue<IGameObject> iGameObjects) {
        this.iGameObjects = iGameObjects;
    }
    protected void approachTo(Position p){
        if (position.x != p.x){
